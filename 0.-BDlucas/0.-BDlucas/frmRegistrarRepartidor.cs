...
namespace _0._BDlucas
{
    public partial class frmRegistrarRepartidor : Form
    {
        ...
        private void btnRegistrar_Click(object sender, EventArgs e)
        {
            ...
            "insert into Repartidor(codRepartidor,tipoVehiculo) values(@IdEntidad,'{4}')" +
            "insert into Direcciones(codEntidad,ubicacion) values(@IdEntidad,'{5}')",
            txbNombre.Text, tbxCelular.Text, tbxEmail.Text, cmbPaisCiudad.Text, cmbVehiculo.Text, tbxDireccion.Text,tbxContrasenia.Text);

            int cantRegistros = GestorDeBaseDeDatos.Instance.EjecutarConsulta(insertNegocio.ToString());
            if (cantRegistros >= 3)
                MessageBox.Show($"Se ingresó {cantRegistros - 3} registro(s) en Repartidor", "Mensaje");
            else
            {
